...
function storeSession(sessionId, email) {
  try {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function verifySession() {
      try {
        const res = await fetch(`/api/session/${encodeURIComponent(storedSessionId)}`);
        if (!res.ok) {
          throw new Error(`HTTP ${res.status}`);
        }
        const data = await res.json();
        ...
        setGuestEmail(data.guest_email);
        storeSession(data.session_id, data.guest_email);
